...
- [2026-09-05] [Dev] **Khôi phục chuỗi PROD lần 2 cho couple `qlAB4LKZCQV7MwB8SvPy`** theo lệnh user (hỏi "có phải đang mất chuỗi không" → inspect → user ra lệnh restore). Nguyên nhân: ngày **2026-08-29** chỉ "Anh By" trả lời ("khoảnh khắc embe đập anh" 12:01Z), "Em Bé Iu" (`thaohathao14@gmail.com`, uid `ZhXTomWO…`) thiếu ⇒ marker KHÔNG có field `bothAnswered` (inspect in `false` là fallback, `[pre]` in `undefined`) ⇒ chuỗi đứt còn 5–6 ngày (runStart 08-30). Lệnh: `node scripts/prod-daily-answer.js restore thaohathao14@gmail.com 2026-08-29 "đã trả lời rồi"` → tạo `responses/ZhXTomWO…` + merge marker `{bothAnswered:true, revealedAt}` (10:49Z, không backdate). Verify inspect sau đó: **92 ngày liên tục 2026-06-05→09-05, gaps=none, longest=92** (state `inProgress`). Tác dụng phụ y hệt lần 08-23: CF `notifyDailyAnswer` bắn 1 inbox+push ngày cũ cho Anh By + huỷ dải nhắc local hôm nay trên máy đó tới khi mở app. 📌 **Gotcha môi trường:** 3 lần đầu fail `getaddrinfo ENOTFOUND identitytoolkit.googleapis.com` dù `dns.resolve4` ra IP (system resolver macOS trả AAAA-only tạm thời, cả trong lẫn ngoài sandbox) — tự hết sau ~1 phút, chỉ cần retry, KHÔNG phải lỗi script/credential.
