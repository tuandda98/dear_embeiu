...
- [2026-06-04] [Designer] Xong design spec shame-free (`design.md`): StreakChip ở footer CounterCard + StreakSheet + Journal summary, 7 state có cơ chế chống-shame, biểu tượng ngọn-lửa-hồng, celebration 5 mốc, copy vi+en đầy đủ, mục phụ thuộc kỹ thuật cho Dev (marker `bothAnswered`, StreakProvider, `longestStreak` trên couple doc, slot `footerExtra`).
