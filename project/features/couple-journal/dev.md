...
- [2026-06-14] [Dev] **Header màn Nhật ký → chip-only** (user kèm ảnh: "đổi chip thành nhật ký câu hỏi, xoá dòng title 'Nhật ký câu hỏi' + dòng subtitle"). ① ARB `journalBadge`: vi "HỎI ĐÁP MỖI NGÀY"→**"NHẬT KÝ CÂU HỎI"**, en "DAILY Q&A"→**"QUESTION JOURNAL"** (chỉ dùng ở màn này) + gen-l10n. ② `SubScreenHeader.title` đổi `required`→**optional** (render có điều kiện) — `_JournalHeader` bỏ `title`+`subtitle` ⇒ còn back + chip + streak summary. `journalScreenTitle`/`journalHeaderSubtitle` giờ unused ở UI (giữ trong ARB). `flutter analyze lib` 0 issue. Client-only.
- [2026-06-14] [Dev] **`JournalScreen` thêm deep-link `focusDate` (YYYY-MM-DD).** Phục vụ tap thông báo `daily_question` từ Notification Center (xem `features/notifications/dev.md`): mở Journal cuộn tới + highlight đúng ngày. `_JournalList` → StatefulWidget; post-frame `Scrollable.ensureVisible` tới `GlobalKey` card khớp `day.date==focusDate` + `AnimatedContainer` viền `accentLove` pulse 2.6s rồi tắt; no-op khi `focusDate` null hoặc ngày chưa nằm trong page đã load (deep-link nhắm hôm nay → luôn card đầu). analyze sạch. Không đụng provider/service/rules.
