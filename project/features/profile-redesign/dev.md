...
## Nhật ký
- [2026-06-18] [dev] **Badge grid visual redesign v2 (nền trắng + accent strip).** Implement `_badgeCard` theo design spec v2: nền trắng đặc (bỏ tint .10), shadow black.06 blur16 offset(0,8), ClipRRect r24, icon 52×52 r16 nền tint.12 size 26 Bold, số màu tint 26px w800 letterSpacing -0.5, label 14px w600, accent strip 4px Positioned đáy gradient [tint.0→tint.60→tint.0], shimmer `ShimmerSkeleton(48,20,r6)` thay Container tự chế, icon `info_circle 14px tint.40` thay arrow_right_3, bỏ border đặc. Stack structure: `Material(transparent) → InkWell(r24, splash tint.10, highlight tint.06) → ClipRRect(r24) → Stack[Container base, Padding content, Positioned accent]`. analyze 0.
