...
- [2026-06-19] [Designer→Dev] **Chip title cao bằng nút back** (user: "chỉnh height icon và title bằng nhau"). Giữ back 44 (touch-target a11y), nâng chip lên 44 qua param mới `EyebrowChip.minHeight` (additive, default giữ nguyên mọi màn khác). **[vòng 2 — user "ko muốn kéo width, muốn wrap content + ở giữa"] FIX:** bản đầu dùng `constraints(minHeight)+alignment:center` → `alignment` khiến Container **giãn FULL width** (sai). Đổi sang `SizedBox(height: minHeight) + Center(widthFactor:1, child)`: cao cố định 44 nhưng **width ôm nội dung** (widthFactor 1 = co width theo child, fill height), KHÔNG `alignment` trên Container. Chip wrap-content + căn giữa (Stack alignment center). analyze 0.
- [2026-06-19] [Designer→Dev] **Read-receipt đổi sang CHỮ kiểu iMessage + GIỜ xem** (user hỏi UX, chọn "Đã xem · 23:30"). `_StatusLabel` bỏ icon → **chữ nhỏ phải-canh** (11px w500): đang gửi/đã gửi/đã nhận = xám nhạt (textTertiary), **đã xem = đậm hơn (textSecondary) + "· HH:mm"** (giờ partner đọc). Giờ lấy từ `ChatProvider.partnerReadAt` (getter mới, = `_partnerReadAt`/receipt `readAt`) → wire `latestMineReadAt` qua build→`_MessageList`→`_StatusLabel`, format `DateFormat.Hm(locale)`. l10n: VI `chatStatusRead` "Đã đọc"→**"Đã xem"** (EN sẵn "Seen") + gen-l10n. Trên ảnh: chữ trắng + shadow .55 (bỏ pill, minimal). analyze 0. ⟪thay cho bản icon-only ngay trước đó⟫.
- [2026-06-19] [Designer→Dev] ~~Read-receipt tối giản icon-only~~ (đã thay bằng bản CHỮ ở trên cùng ngày — user chọn kiểu iMessage có giờ). **Read-receipt tối giản kiểu Messenger** (user: "chỉnh màu đã đọc/đã nhận/đã gửi tối giản giống fb"). BỎ chữ + BỎ viên scrim → chỉ còn **1 icon 14px** tiến triển: sending=`clock` (tertiary mờ) · sent=`tick_circle` linear/rỗng (tertiary) · delivered=`tick_circle` bold/đặc (secondary xám) · read=`tick_circle` bold. **[vòng 2 cùng ngày — user "không muốn check màu đỏ"] BỎ rose hẳn:** read đổi `accentLove`→`textPrimary` → dải XÁM trung tính tăng dần tertiary→secondary→primary, read = tick đậm/đầy nhất, KHÔNG màu. Trên ảnh: icon trắng theo brightness (read alpha 1.0 · delivered .85 · còn lại .7) + shadow mảnh, KHÔNG pill (đúng phong cách FB icon trần). A11y giữ qua `Semantics(label: <phrase l10n>)`. `_TimeDivider` (date stamp "0:17") GIỮ scrim pill — concern khác, user không nhắc. analyze 0.
- [2026-06-19] [Designer→Dev] **Fix legibility "chữ trên ảnh nền đổi liên tục"** (user: divider giờ "12 THG 6 14:10"… + "Đã nhận" mờ trên ảnh nền): màu cố định kiểu gì cũng có vùng ảnh nuốt chữ. Giải pháp độc lập ảnh = **viên scrim tối mờ (`Colors.black` .38, bo 999) + chữ TRẮNG** cho `_TimeDivider` + `_StatusLabel` KHI `hasBackground` (đọc được trên mọi vùng ảnh sáng/tối/rối — chuẩn WhatsApp/Telegram stamp trên wallpaper). Trạng thái "đã đọc" giữ accentLove (rose nổi trên scrim). Khi nền gradient (không ảnh) → giữ navy như cũ. Threading: `hasBackground` xuống `_MessageList` → `_TimeDivider.onBackground`/`_StatusLabel.onBackground`. Bong bóng navy/trắng vốn đặc nên không cần đụng. analyze 0.
- [2026-06-17] [Designer→Dev] (vòng 2, user "đậm hơn nữa + ở giữa") **Canh chip ra GIỮA** header row: đổi Row→Stack(center) y hệt `SubScreenHeader` — chip couple-name dead-centre, back arrow ghim mép trái (`Transform.translate(-10)`), padding ngang 44 chống tên dài đè lên back. Chip `AnimatedCoupleName` thêm `alignment: WrapAlignment.center`. **"Đậm hơn":** font Quicksand kịch trần ở **Bold (700)** — `w800` đã resolve về đúng file Bold, KHÔNG nặng hơn được nữa bằng weight → dùng **cỡ làm đòn bẩy**: 13→15pt (bằng cỡ chữ tin nhắn nhưng ALL-CAPS+Bold+tracked nên vẫn áp đảo), tracking 1.2→1.0, heart 13→15.
- [2026-06-17] [Designer→Dev] Phân biệt rõ "giọng chữ" header vs tin nhắn (user: header chip "ANH TUAN ❤ EM" trông giống bong bóng tin nhắn trắng). Header couple-name chip nâng từ eyebrow nhạt → **tiêu đề mạnh**: ALL-CAPS, navy ĐẶC 100% (bỏ alpha .70 chìm), w800 (was w700), 13pt (was 11), letterSpacing 1.2, heart 13. Tin nhắn GIỮ giọng nói tự nhiên (sentence-case 15/w400). Tách 3 trục cùng lúc: hoa-vs-thường · đậm-vs-nhạt · giãn-vs-liền. KHÔNG đổi vỏ EyebrowChip (giữ luật 4-tab chip-only §8) — chỉ tinh chỉnh `textStyle` của `AnimatedCoupleName` trong `chat_screen.dart`. analyze 0.
