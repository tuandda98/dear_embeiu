...
- [2026-06-04] [Dev] FIX `daily_reveal` Lottie **kẹt frame cuối (ngôi sao đỏ) hiện liên tục** trên card Daily Question (user báo). Root cause: render gate `if (hasRevealed)` — state này đứng yên=true khi cả 2 đã trả lời → Lottie hiện mọi lần mở card; `repeat:false` lại giữ frame cuối (Polystar) → sao đứng yên đè lên chữ. Fix: biến thành **one-shot tự ẩn** — thêm `_showRevealLottie` (gắn cùng one-shot guard của confetti, init theo `hasRevealed` nên mở lại card đã reveal KHÔNG hiện), bật khi vừa mở khoá rồi `Future.delayed(2600ms)`→`setState(false)`. Render đổi sang `if (_showRevealLottie)`. `home_screen.dart` (`_DailyQuestionCardState`). VERIFY runtime Pixel 10: mở lại card đã reveal → hết sao kẹt. analyze sạch. (One-shot lúc reveal tươi: verify logic, cần fresh both-answer để xem live.)
