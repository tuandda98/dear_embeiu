...
- [2026-09-05] [Dev] **Vá theo code-review + Tester (catch-up gate):** `submitAnswer(backfill: true)` ghi cờ lên response ngày cũ → CF `notifyDailyAnswer` vẫn stamp `bothAnswered` nhưng BỎ push/inbox/wake (hết tác dụng phụ "hôm nay" + huỷ nhắc người ấy); `CatchupService.findMissedDays` chỉ tính ngày người ấy ĐÃ trả lời & mình chưa, trả `null` khi lỗi (caller giữ dải nhắc, bỏ throttle); gate: timeout 12s → coi như xếp hàng + đi tiếp, bỏ `enabled:!_sending` (mất focus), `dispose` reset `_isShowing`; `ReminderProvider` reset `_personalCatchupSignature` ở nhánh non-target + dùng `CatchupService.gatedEmail` làm nguồn duy nhất. Marker `dailyAnswers/{date}` nay ghi bằng `runTransaction` và **rules khoá `questionVi/En` bất biến khi đã tồn tại** (chặn app 1.5.0 ghi đè câu engine; +3 rules-test → 241). ✅ DEV deploy `firestore:rules,functions:notifyDailyAnswer,functions:generateDailyQuestion` trace `20260905T120412Z`. Script `prod-daily-answer.js restore` cũng ghi `backfill:true`.
- [2026-09-12] [Dev] **Vá 10 finding `/code-review max` (cụm câu hỏi hằng ngày):** (1) 🔒 rules `answerReactions` pin thêm `answerAuthorUid in memberIds` + CF `notifyDailyAnswerReaction` kiểm tra membership FAIL-CLOSED (trước đó member ép được push/inbox tới uid lạ; +1 rules-test → 244). (2) `DailyQuestionProvider.submit`: snapshot `_resolved` TRƯỚC guard midnight-rollover, gọi `_maybeResolveToday` SAU khi answer land (hết ghi câu bank sai + hết race marker). (3) `QuestionEngine._publish` catch nay set `_publishFailed` thật; `ResolvedQuestion.published` → provider retry throttle 60s khi chưa publish được. (4) CF `notifyDailyAnswer` data thêm `date`; client `_cancelStaleDailyQuestionNudges` chỉ huỷ band khi `date == hôm nay`; Home resume gọi `invalidateDailyQuestionSchedule()` + re-arm. (5) `_DailyAnswerSheet._submit` try/catch → SnackBar `dailyQuestionSubmitError`, spinner không kẹt. (6) `updateContext(signalsReady:)` giữ resolve tới khi `StreakProvider`/`MoodProvider` (thêm `isLoading`) có snapshot; `_refreshDqSafetyNet` listen thêm mood + re-sync context. (7) `BankQuestionSource.isExhausted` + lọc id ngoài range; engine `record(resetAskedBankIds:true)` khi cạn → cycle mới no-repeat (+2 test). (8) `cancelDailyQuestionSchedule()` huỷ luôn 3 band personal 1100–1159 + reset debounce (sign-out không còn nổ "Anh By…"). (9) `StreakProvider.recomputeIfDayChanged()` từ snapshot cache, gọi ở `_refreshDqSafetyNet` + resume. (10) bên thua race publish cũng qua `_withRevisitHint`. Nhỏ: copy `loveTreeMomentPhotosCopy` "hai đứa"→"chúng mình", comment band backstop 1020–1033. analyze 0 · test 83/83 · rules-test 244. ✅ DEV deploy `firestore:rules,functions:notifyDailyAnswerReaction,functions:notifyDailyAnswer`. ⚠️ **PROD CHỜ LỆNH** cùng lệnh `--project prod` (rules additive-siết có chủ đích: chỉ chặn author segment ngoài couple). Client-only phần còn lại → cần release 1.6.2.
