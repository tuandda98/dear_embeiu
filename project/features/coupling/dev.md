...
## Nhật ký implement
- [2026-06-29] [Dev/Lead] **Fix status parity khi ghép đôi: A (người tạo) kẹt `waiting_partner` dù couple đã active.** Root cause: transaction join (`couple_service.joinCoupleByCode`) chỉ ghi doc của NGƯỜI JOIN (B → `status:'in_couple'`) + couple→`active`; **A không bao giờ được sửa** (transaction không ghi được doc A, A thường offline; watcher couple bên A chỉ cập nhật `_couple` in-memory; CF `notifyPartnerJoined` chỉ push, không ghi status; login lại đọc đúng doc Firestore vẫn stale — chỗ tính `in_couple` ở `auth_service:640` chỉ là offline-fallback object, KHÔNG ghi DB) → `users/{A}.status` đứng mãi `waiting_partner`. **Rules KHÔNG sai** — `canUpdateOwnUser` đã cho phép `status` (allowlist dòng 84); thiếu là KHÔNG có code nào thực hiện cú ghi cho A. Tác động: routing không vỡ (theo `coupleId`+`couple.status`) nhưng dữ liệu lệch + rủi ro latent: guard join `couple_service:358` thấy A `waiting_partner` → nếu A quay lại nhập mã sẽ `leaveCouple()` phá couple active; analytics `coupleStatus` sai. **Fix 2 lớp:** (A) client self-heal — `CoupleService.reconcileActiveStatus` (A ghi DOC CỦA CHÍNH MÌNH → pass rules, khác cú ghi cross-user của leave-case vốn bị deny) + wire vào watcher `couple_provider` (latch `_statusHealAttempted` 1 lần/load) + callback `onMemberStatusHealed` ở `session_resolver` đẩy user đã heal về `authProvider.updateCurrentUser` (đồng bộ in-memory, idempotent — pass sau short-circuit khi `status=='in_couple'`); (B) CF authoritative — `notifyPartnerJoined` thêm `users/{recipient}.set({status:'in_couple'}, merge)` admin-SDK (bỏ qua rules), best-effort per doc. Self-heal client cũng lành các couple LEGACY khi A mở app. analyze 0 (lib), rules-test 187 pass (CF không đụng rules). **✅ CF `notifyPartnerJoined` ĐÃ deploy DEV + PROD 2026-06-29** (cùng đợt với `notifyPartnerLeft` dưới).
- [2026-06-29] [Dev/Lead] **Đối xứng chiều LEAVE: CF `notifyPartnerLeft` ghi B về `waiting_partner`.** Cùng class bug nhưng chiều ngược: khi A rời (hoặc xoá tài khoản → demote admin-side), client `couple_service.leaveCouple:676` cố ghi `users/{B}.status='waiting_partner'` nhưng là cú ghi CROSS-USER → `canUpdateOwnUser` deny (rơi catch, best-effort) → B kẹt `'in_couple'`. Fix: thêm khối heal vào `notifyPartnerLeft` — `users/{recipient}.set({status:'waiting_partner'}, merge)` admin-SDK (bỏ qua rules), best-effort per doc, đặt ngay sau guard leaver/recipient. Phủ luôn nhánh account-deletion (CF này cũng fire khi `deleteAccount` demote couple). node load OK, rules-test 187 pass (CF không đụng rules). **✅ ĐÃ deploy DEV + PROD 2026-06-29** (cùng đợt `notifyPartnerJoined`). ⚠️ **Bài học deploy:** firebase-tools SKIP NHẦM hàm thứ 2 khi deploy CHUNG 2 hàm cùng-trigger trong 1 lệnh (dedup theo hash bundle — `notifyPartnerLeft` bị bỏ qua dù hash khác, verify REST: hash cũ + updateTime cũ). **PHẢI deploy TỪNG hàm 1 lệnh riêng** (`--only functions:notifyPartnerLeft` rồi `--only functions:notifyPartnerJoined`) + verify updateTime mới qua Cloud Functions REST (`labels.firebase-functions-hash` + `updateTime`). Auto-trace hook KHÔNG snapshot deploy functions-only (chỉ rules) → restore = git revert `functions/index.js` + redeploy. Ghi chú: in-memory `currentUser.status` của B vẫn stale trong-phiên đến lần launch sau (đọc Firestore đã được CF sửa); rủi ro latent nhỏ (guard join `couple_service:340` chặn B nhập mã mới đến khi re-login) — nếu cần triệt để in-memory có thể mở rộng client self-heal đối xứng (CHƯA làm, ngoài scope yêu cầu).
