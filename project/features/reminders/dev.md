...
- [2026-09-07] [dev] **BỎ HẲN nhắc uống thuốc → đổi thành lời hỏi thăm hằng ngày** (user: "bỏ uống thuốc rồi, nhắc ăn sáng, hỏi thăm hôm nay, có nhớ anh không, đang làm gì… đừng lặp lại, xào random mỗi ngày khác nhau"). Vẫn account-gated `thaohathao14@gmail.com`, vẫn band **1100–1109**, vẫn daily-recurring (`DateTimeComponents.time`) — chỉ đổi ngữ nghĩa + nội dung + giờ.
  - Đổi tên API cho khớp: `ReminderService.schedulePersonalMedicineDaily`→**`schedulePersonalCareDaily`**, `cancelPersonalMedicine`→**`cancelPersonalCare`**, `_idPersonalMedicineBase`→`_idPersonalCareBase`, `_maxPersonalMedicine`→`_maxPersonalCare`; provider `_personalMedicineDayKey`→`_personalCareDayKey`.
  - **Giờ đổi 9:59/10:10/10:30 → 7:45 (sáng) · 13:30 (trưa) · 20:30 (tối)** cho khớp ngữ cảnh từng buổi.
  - **3 pool riêng theo buổi, mỗi pool 15 bộ (title, body)** — sáng = nhắc ăn sáng + chúc ngày mới · trưa = hôm nay thế nào / đang làm gì / ăn trưa chưa · tối = có nhớ anh không / kết ngày ấm áp. Title luôn mở đầu "Anh By"/"Anh yêu".
  - **Xoay không lặp:** `_rotatingIndex(dayIndex, seed, len) = (dayIndex*7 + seed*5) % len`; bước 7 nguyên tố cùng 15 ⇒ mỗi pool duyệt HẾT 15 câu rồi mới quay lại (verify: 15 ngày ra 15 index khác nhau ở cả 3 khung), `seed`=chỉ số khung làm 3 buổi lệch pha ⇒ trong ngày 3 lời khác chủ đề, 2 ngày liền nhau không trùng câu.
  - ⚠️ Vẫn re-arm tối đa 1 lần/ngày khi app được mở (debounce `_personalCareDayKey`); ngày em ấy không mở app thì 3 noti vẫn nổ nhưng giữ nội dung của lần arm gần nhất.
  - Client-only, KHÔNG đụng backend. analyze 0 · test 81/81. **Cần phát hành bản mới thì máy em ấy mới nhận (bản store hiện tại vẫn đang nhắc uống thuốc).**
