...
**Screens (`lib/screens/`):** splash, auth_gate, login, register, setup, home, gallery, profile. Widgets ở `lib/widgets/`. Theme ở `lib/theme/`.
...
- `sendPartnerPhotoNotification` — onDocumentCreated `couples/{coupleId}/photos/{photoId}`. Khi đăng ảnh, gửi FCM cho **partner** (member khác author). Title hardcode VI: `{authorName} vừa đăng ảnh mới 💞`; body = caption hoặc fallback. Android channel `partner_photo_updates`. Xoá token invalid sau gửi.
...
- **Love reminders — local only** (không network): `reminder_provider.dart` (what — Hive `reminder_settings`) + `reminder_service.dart` (how — channel `love_reminders`, id 1001-1005, text từ `AppL10n.strings`). 4 loại: daily / anniversary / milestone (mỗi 100 ngày + nudge trước 3 ngày) / inactivity (7 ngày).
...
**Components tái dùng (lib/widgets):** counter_card (HERO gradient sunsetRomance bo 28), animated_couple_name ("P1 ♥ P2" heart pulse 820ms), shared_couple_photo_view/shared_photo_view (smart loader local→network→fallback), blocking_loading_overlay, language_toggle_button, photo_item + masonry_gallery (⚠️ MasonryGallery hiện KHÔNG dùng — gallery thực tế là feed dọc), auth_background (định nghĩa nhưng không dùng).
...
- **profile_screen** — header eyebrow + **hero card couple** (bo 32, ảnh nền blur/gradient + initials 56px, badge glass đếm ngày tới kỷ niệm, avatar 72px viền gradient, tên 30px serif w800). Stats 2×2 (years/months còn lại/total days/memories). Info tiles (start date, milestone, invite code). Reminders toggle (Switch.adaptive + time picker). Language picker row. Nút "Edit our story" → setup. **Danger zone** bo 28 viền đỏ: clear cache → leave couple → divider "Irreversible" → delete account. Link privacy dưới cùng.
